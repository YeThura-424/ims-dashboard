      ...
      <el-card class="box-card">
        <el-form :model="importForm" label-width="120px" label-position="left">
          <el-form-item label="Import Code">
            <el-input v-model="importForm.importcode" />
          </el-form-item>
          <el-row>
            <el-col :span="11">
              ...
              <el-form-item label="Product Photo">
                <el-tabs v-model="activeName" class="demo-tabs">
                  ...
                  <el-tab-pane label="Old Photo" name="second">
                    <div class="img-preview" v-if="importForm.oldPhoto">
                      <img
                        v-if="importForm.oldPhoto"
                        :src="importForm.oldPhoto"
...
watch(
  [
    ...
    () => mrnStore.currentImport.data,
  ],
  ([newQty, newRate, newImportItem, newData]) => {
    ...
    importForm.importcode = newData.import_code;
    importForm.date = newData.import_date;
    importForm.invoicedate = newData.import_invoice_date;
    importForm.invoiceno = newData.import_invoiceno;
    importForm.vendor = newData.import_vendorid;
    importForm.remark = newData.import_remark;
    importForm.oldPhoto = newData.import_photo;
    importForm.id = newData.import_id;
  }
);
onMounted(async () => {
  await mrnStore.getImport(route.params.id);
  vendorStore.getVendors();
  tableData.importItem = mrnStore.currentImport.product;
  localStorage.setItem(
    "importDetailProducts",
    JSON.stringify(tableData.importItem)
  );
  ...
  if (storedProduct) {
    ...
    importForm.totalamount = tableData.importItem.reduce((total, item) => {
      return total + item.productamount;
    }, 0);
  }
});
